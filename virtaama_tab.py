...
from python_vesla_virtaama import (
    vedenkorkeus_API,
    pivot_ja_tallennus,
    prosentti_pisteet,
    pysyvyydet,
...
class VirtaamaTab:
    ...
    def filter_stations(self):
        ...
        if self.active_var.get():
            df = df[df["Tila"] == "Jatkuva"]
        if self.maakunta_var.get() != "All":
            df = df[df["Maakunta"] == self.maakunta_var.get()]
        typed = self.station_var.get().lower()
        if typed:
            df = df[df["display"].str.lower().str.contains(typed)]
        self.station_combo["values"] = df["display"].tolist()
    ...
    def run_analysis(self):
        try:
            ...
            df = vedenkorkeus_API(start, end, station_id)
            if df.empty:
                raise ValueError("Ei dataa")

            variable = "W N2000" if "W N2000" in df.columns else "Arvo"
            df_pivot = pivot_ja_tallennus(df, havainto_paikka=variable, vakio=0.01)

            if self.tallennus_var.get() and self.save_path1:
                df.to_excel(self.save_path1, index=False)

            if self.pivot_var.get() and self.save_path:
